...
```
✗ 誤り：「オッズ比2.0は確率が2倍になること」

✓ 正しい：オッズ比は「オッズの比」であり確率の比ではない

  例：基準の確率が p = 0.1 のとき
      オッズ = 0.1/0.9 = 0.111
      オッズ比 = 2.0 なら新オッズ = 0.222
      新確率 = 0.222/(1+0.222) = 0.182

      確率は 0.1 → 0.182（1.82倍、2倍ではない）

  確率が小さい（まれな事象）ときは
  オッズ比 ≈ 確率の比（リスク比）と近似できる
```

### 誤解2：ロジスティック回帰での R²

```
✗ 誤り：「ロジスティック回帰でも R² で適合度を評価」

✓ 正しい：通常のR²は GLM では使えない
          代わりに「疑似R²」を使う

  疑似R²の種類：
  ・McFadden R² = 1 - (対数尤度_model / 対数尤度_null)
  ・Nagelkerke R²
  ・Cox-Snell R²
...
```
✗ 誤り：「ポアソン回帰は平均と分散が等しいと仮定」

✓ 正しい：ポアソン分布の性質上 E[Y] = Var(Y) = μ だが、
          実データでは過分散（Var > E）が多い
...
```
✗ 誤り：「二項分布にはロジットリンクを使うべき」

✓ 正しい：標準（正準）リンクが常に最適とは限らない

  二項分布で使えるリンク関数：
  ・ロジット：log(p/(1-p))  ← 最も一般的
  ・プロビット：Φ⁻¹(p)      ← 潜在変数解釈
  ・補対数対数：log(-log(1-p)) ← 非対称

  選択の基準：
  ・解釈のしやすさ（オッズ比が欲しいならロジット）
  ・データへの適合度
  ・理論的背景
```
