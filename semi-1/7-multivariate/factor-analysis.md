...
PCA との違い：
  PCA：x → z（観測値を主成分に変換）
  因子分析：f → x（因子が観測値を生成、誤差を考慮）
```

**SEM** = **S**tructural **E**quation **M**odeling（構造方程式モデリング）
**CFI** = **C**omparative **F**it **I**ndex（比較適合度指標）
**RMSEA** = **R**oot **M**ean **S**quare **E**rror of **A**pproximation
