...
| 元の分布・操作 | 結果の分布 |
|-------------|----------|
| $Z \sim N(0,1)$ の二乗 | $\chi^2(1)$ |
| 独立な $Z_i^2$ の和（$n$個） | $\chi^2(n)$ |
| $Z/\sqrt{V/n}$（$V \sim \chi^2(n)$） | $t(n)$ |
| $(\chi^2_m/m) / (\chi^2_n/n)$ | $F(m,n)$ |
| $t(n)^2$ | $F(1,n)$ |
| $(n-1)S^2/\sigma^2$（正規母集団） | $\chi^2(n-1)$ |
| 独立な $\text{Exp}(\lambda)$ の和（$k$個） | $\Gamma(k, \lambda)$ |
| $X/(X+Y)$（$X,Y$は独立ガンマ） | $\text{Beta}$ |
