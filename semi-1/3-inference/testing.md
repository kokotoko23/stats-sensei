...
```
Step 1: 対数尤度のテイラー展開
  ℓ(θ̂₀) を θ̂（制約なしMLE）のまわりで展開：
  ℓ(θ̂₀) ≈ ℓ(θ̂) + (θ̂₀ - θ̂)ᵀ∇ℓ(θ̂) + (1/2)(θ̂₀ - θ̂)ᵀH(θ̂)(θ̂₀ - θ̂)

  ∇ℓ(θ̂) = 0（MLEの定義）より：
  ℓ(θ̂₀) - ℓ(θ̂) ≈ (1/2)(θ̂₀ - θ̂)ᵀH(θ̂)(θ̂₀ - θ̂)
...
Step 4: MLEの漸近分布
  ...
  H₀: θ = θ₀ のもとで、r個の制約があるとき：
  (θ̂₀ - θ̂) は r 次元の自由度をもつ

Step 5: 二次形式の分布
  正規分布の二次形式 → χ² 分布

  -2logΛ ≈ (θ̂₀ - θ̂)ᵀ[nI(θ)](θ̂₀ - θ̂)
         →ᵈ χ²ᵣ

  ※ r = 制約されるパラメータの数（自由度）
```

**直感**：尤度比検定統計量は、MLEの漸近正規性を通じてχ²分布に従う
