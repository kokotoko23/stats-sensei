...
頻度主義は「パラメータは固定、データが変動」と考えます。ベイズ主義は逆に「データは固定、パラメータが確率変数」と考えます。
...
頻度主義：
  θ は固定された未知の値
  「θ = 0.5 である確率」という表現は無意味

ベイズ主義：
  θ は確率分布に従う確率変数
  「データを見た後、θ が 0.4〜0.6 にある確率は95%」と言える

ベイズの更新プロセス：
  事前分布 p(θ)  ──→  データ x を観測  ──→  事後分布 p(θ|x)
    「最初の信念」        「証拠」         「更新された信念」
...
**MCMC** = **M**arkov **C**hain **M**onte **C**arlo（マルコフ連鎖モンテカルロ法）
